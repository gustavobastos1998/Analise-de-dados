...
def orders_by_day(df):
    """bar plot of the count of IDs grouped by order date"""
...
def ids_by_city_road_density(df):
    """scatter plot of the count of IDs grouped by city and road traffic density"""
...
def percentage_by_traffic_density(df):
    """pie plot of the count of IDs grouped by road traffic density"""
...
def orders_by_week(df):
    """plot of the count of IDs grouped by week of year"""
...
def median_points_city_traffic_density_map(df):
    """median point of latitudes and longitudes ploted on a map"""
...
with tab1:
    ...
    with stl.container():
        ...
        with col1:
            ...
            fig = ids_by_city_road_density(df)
            stl.pyplot(fig,use_container_width=True)
        with col2:
            stl.markdown('Percentage of deliveries by traffic density')
            fig = percentage_by_traffic_density(df)
            stl.pyplot(fig, use_container_width=True)
...
with tab3:
    ...
    map = median_points_city_traffic_density_map(df)
